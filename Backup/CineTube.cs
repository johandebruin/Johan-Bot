            ...
            while ((ch = analizador.html.Read()) != -1)
            {
                if (ch == 0)
                {
                    ...
                    if (analizador.html.Tag.Name == "a" && enlace == null)
                    {
                        ...
                        descripcion = MiembrosEstaticos.Extraer(codigo, "class=\"cover\" align=\"left\">SINOPSIS: ", "</div>", i++).Replace("á", "a").Replace("é", "e").Replace("í", "i").Replace("ó", "o").Replace("ú", "u").Replace("ñ", "n").Replace("É", "E").Replace("Á", "A").Replace("Í", "Í").Replace("Ó", "O").Replace("Ú", "Ú").Replace("'", "");
                        ...
                        if (embed != null)
                            Agregar.Añadir(titulo, "17", titulo + ".jpg", embed, Servidor, descripcion);
